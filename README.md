...
* 存在しない日付(月・日の範囲超過、存在しない閏月、改暦により存在しない明治5年12月3日〜31日など)は例外(ArgumentError)を上げます。
* 北朝の元号も解釈できます。しかし現在の所、北朝の元号で文字列にフォーマットすることはできません。Date や ユリウス日からの変換では南北朝期は南朝の元号になります(南北朝合一後は明徳)。
