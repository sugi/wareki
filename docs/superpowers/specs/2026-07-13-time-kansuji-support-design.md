...
3. `Time.parse("平成元年5月4日 二十五時")` は従来 00:00:00 として通っていたが、
   ...
   承認済みの線引き)
4. 「10時間耐久」「13時代」のような「数字+時」を含む複合語(時間量・時代など)も
   時刻として翻字されるため、従来 Date.parse が数字を日として拾って成功していた
   文字列が ArgumentError になる場合がある(レビューで検出後、翻字方式の帰結として
   維持することを 2026-07-13 に維持者が承認。複合語除外の否定先読み案は不採用)
5. 既存の Date の挙動・%J 指示子はすべて不変
